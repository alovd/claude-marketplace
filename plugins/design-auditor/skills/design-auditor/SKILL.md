...
_"Inferred: Full audit · Dev handoff · WCAG AA — let me know if any of these are wrong."_
...
- options: "Full audit (default)" / "Quick audit — 5 categories" / "Custom categories" / "Early concept" / "Dev handoff (default)" / "Production" / "WCAG AAA (default is AA)"
...
State at top of report: _"Quick audit — 5 categories selected for your [type]. Run a full audit to check all 17."_
...
_"Detected: Form — auditing 12 of 17 categories. Skipped: i18n & RTL, Navigation, Responsiveness, Motion, Design Tokens (no code provided)."_
...
_"I can see you've shared a Figma link, but I don't have Figma MCP access in this session. Could you export a screenshot or paste the relevant CSS/component code? I can still run a full audit — I'll just note it as 🟡 Medium confidence since I won't have exact layer data."_
...
**Use for Category 2 (Color & Contrast):**

Compute WCAG 2.1 contrast ratios for all foreground/background token pairs. Look for naming patterns like `color/text/*` paired with `color/background/*`, `color/on-*` paired with `color/*`, `color/foreground` paired with `color/canvas`. Flag failures against the selected WCAG level. Pre-populate the Contrast Checker widget with the exact failing hex pair and the computed ratio.

Also flag if no text/background token pairs can be identified — this means contrast cannot be verified from tokens alone.

**Confidence upgrade:** If `get_variable_defs` returns usable color pairs, the Cat 2 audit upgrades from 🟡 Medium to 🟢 High confidence even if no screenshot is available. State: _"Color contrast audited from design tokens (no screenshot required) — 🟢 High confidence."_
...
1. Identify the failure type ("Node not found" → re-call get_design_context; "Cannot edit instance" → find main component; "Invalid operation" → check node type; "Permission denied" → report view-only; Unknown → report and skip)
2. Report: "⚠️ Fix [N] failed: [reason]. Skipping to the next issue — I'll note this one so you can apply it manually."
3. Log failed fixes. After loop completes: "N fixes applied ✅. N fixes need manual attention:" with exact Figma right-panel values.
4. Never stop the entire fix loop because one fix failed.
...
- Do not cite exact hex values — describe color relationship instead
- Skip categories impossible to assess visually: Design Tokens, exact Typography metrics
- Add a banner: _⚠️ **Medium confidence audit** — input was a screenshot. Values are estimated from visual inspection. For an exact audit, share the Figma file or component code._
- Apply a **−50% deduction modifier** to all 🟡 Warning and 🟢 Tip issues that depend on exact values. Only 🔴 Critical visual issues take full deductions.
...
Scoring is defined in the output template below — apply it when generating the report.
...
→ Widget: **Type Scale Stack** — render detected font sizes with roles, flag duplicates and sub-minimum sizes.
...
→ Widget: **Contrast Checker** — pre-populate failing fg/bg hex pair, show all 5 WCAG pass/fail levels, calculate nearest passing hex as fix suggestion. When contrast was calculated from tokens, show token names alongside hex values.
...
→ Widget: **8pt Grid Visualizer** — show offending value on a ruler with valid grid neighbours, snap distance, and on/off-grid coloring.
...
- [ ] **Nested radius rule** — When an element sits inside another, outer radius = inner radius + padding. Mismatched nesting makes corners look "poking out."
- [ ] **Size-proportional radius** — Larger elements need larger radii.
...
- [ ] **Empty state** — What does an empty list, inbox, or dashboard look like? Should include an illustration or icon, a friendly explanation, and a clear next action.
- [ ] **Error state** — Network failures, server errors, and not-found pages need their own designed state.
- [ ] **Partial failure** — What if only some data loads? Design for partial states, not just all-or-nothing.
- [ ] **Success state** — After a form submission or action, confirm it worked.
- [ ] **Disabled state** — Disabled buttons and inputs should look visually distinct and ideally explain why they're disabled.
- [ ] **Consistency** — Loading/empty/error states should match the overall visual style — not be plain browser defaults.

→ Widget: **States Coverage Map** — pre-populate grid with components from the audit, mark states as present/missing/N/A. Always render even for a single missing state.
...
- [ ] **Button labels are verbs** — "Save Changes", "Send Message", "Delete Account" — not "OK", "Submit", or "Yes".
- [ ] **Error messages are human** — "Please enter a valid email address" not "Invalid input".
- [ ] **Placeholder text is not a label** — Placeholders disappear on typing. They can hint at format but never replace a label.
- [ ] **Destructive actions are explicit** — "Delete 'Project Alpha'? This can't be undone." Never just "Are you sure?"
- [ ] **Consistent terminology** — Don't call the same thing "workspace", "project", and "board" interchangeably.
- [ ] **Tone consistency** — Pick a tone and maintain it everywhere.
- [ ] **No lorem ipsum in shipped designs** — Real content often reveals layout problems that lorem ipsum hides.
- [ ] **Empty states have personality** — "Looks like nothing's here yet — add your first task to get started!" beats "No results found."

---

### CATEGORY 13: Internationalization & RTL Support (if applicable)

_Only audit this category if the product targets multiple languages or RTL locales. Read `references/i18n.md` for full guidance._

- [ ] **No hardcoded strings** — All visible text should come from a translation file.
- [ ] **Text expansion budget** — German/Finnish can be 30–40% longer than English. Buttons and labels must accommodate.
- [ ] **RTL layout mirroring** — Use `dir="rtl"` and CSS logical properties (`margin-inline-start` instead of `margin-left`).
- [ ] **RTL-safe icons** — Directional icons (arrows, chevrons) must flip in RTL. Non-directional (heart, star, trash) stay the same.
- [ ] **Date, time & number formats** — Use locale-aware formatting (e.g., `Intl.DateTimeFormat`).
- [ ] **Currency & units** — Symbol position and decimal separators differ by locale.
- [ ] **No text in images** — Images with embedded text can't be translated. Use CSS overlays or separate text layers.
- [ ] **Font support** — Does the chosen font support all target scripts?
...
- [ ] **Shadow scale** — Shadows should come from a defined scale (sm, md, lg, xl) — not arbitrary values.
- [ ] **Shadow = elevation** — Cards sit low, modals sit high, tooltips highest. Check the hierarchy makes sense.
- [ ] **Shadow color** — Use a dark, slightly saturated color (e.g. `rgba(0,0,0,0.08)`) — never pure black. Tint with surface color on colored backgrounds.
- [ ] **No shadows in dark mode** — Use lighter surface colors for elevation instead.
- [ ] **No decorative shadows** — Shadows only on elevated elements.
- [ ] **Consistent blur & offset** — A consistent offset-to-blur ratio makes shadows feel physically grounded.
- [ ] **Multiple light sources** — Don't combine top-shadow and bottom-shadow on the same element unless intentional.
...
- [ ] **Consistent icon family** — All icons from the same set. Never mix outline from one library with filled from another.
- [ ] **Consistent style within family** — All outline, all filled, or all duotone. Don't mix.
- [ ] **Optical sizing** — Standard grid-friendly values: 16, 20, 24, 32, 40, 48px. Avoid in-between values.
- [ ] **Stroke weight consistency** — Don't use 1px icons next to 2px icons.
- [ ] **Touch target padding** — Icons as buttons need padding to reach 44×44px minimum.
- [ ] **Icon meaning consistency** — Same icon = same meaning everywhere.
- [ ] **Label pairing** — Icons without labels are ambiguous. Always pair with label or tooltip. Exception: universally understood icons (✕ close, ☰ menu, ⌕ search).
- [ ] **Optical alignment** — Align icons to optical center, not bounding box edge.
...
- [ ] **Clear current location** — Active nav items must be visually distinct (color, weight, indicator bar).
- [ ] **Tabs vs nav** — Tabs switch views of same content. Nav moves between sections. Don't mix them.
- [ ] **Breadcrumbs for depth** — Any page more than 2 levels deep needs breadcrumbs.
- [ ] **Back button behavior** — "Back" should go to previous screen, not previous URL. Should not close entire flow unexpectedly.
- [ ] **Mobile navigation** — Bottom nav for 3–5 primary destinations. Hamburger for secondary only.
- [ ] **Active state contrast** — Active/selected items must meet 3:1 contrast ratio against inactive items.
- [ ] **Overflow handling** — Too many nav items? Scroll horizontally or collapse into "More" dropdown.
- [ ] **Navigation consistency** — Same items, same order, same style on every page.
...
- [ ] **Colors are tokenized** — No hardcoded hex values in components.
- [ ] **Spacing is tokenized** — Spacing values reference a scale token, not arbitrary pixel values.
- [ ] **Typography is tokenized** — Font size, weight, and line-height come from defined text style tokens.
- [ ] **Radius is tokenized** — Corner radius values reference the radius scale.
- [ ] **Shadow is tokenized** — Box shadows reference elevation tokens.
- [ ] **Token naming is semantic** — Tokens describe _purpose_, not appearance. `color.background.danger` is good. `color.red.500` used directly is not.
- [ ] **No magic numbers** — Any value that appears more than twice should be a token.
- [ ] **Dark mode uses the same tokens** — Dark mode should swap token values, not introduce new hardcoded colors.
...
Scoring: 🔴 Critical = −8pts (breaks usability/accessibility), 🟡 Warning = −4pts (weakens design), 🟢 Tip = −1pt (polish). Floor is 0.
If 🟡 Medium confidence, apply −50% modifier to warnings/tips that depend on exact values.

### Accessibility Score: [X/100]
Combines Categories 2, 6, 7, and 16 only. Same formula.
Bands: 90–100 = WCAG compliant, 70–89 = minor gaps, 50–69 = significant gaps, <50 = failing.
...
Render the **Issue Priority Matrix** widget. Plot every issue as a dot by effort vs impact.
Effort: 1=single value change, 10=architectural overhaul. Impact: 1=cosmetic, 10=blocks accessibility/usability. Use severity as proxy: Critical=high impact, Tip=low impact.
Use deterministic positioning. Render severity as both color AND a letter (C/W/T) for colorblind accessibility.
```

### Step 3b: Radar Chart Visualizer (always run after report)

Immediately after presenting the markdown report, use the Visualizer tool to render an interactive radar chart widget.

**How to generate it:**

- Extract the per-category scores (X/10) from the audit you just ran
- Use only the categories that were actually audited
...
Store the current overall score in a JS variable. If a previous score exists (from a re-audit), show a delta badge next to the centre score: `+N ↑` in green (improved), `−N ↓` in red (dropped), omit if no change.

Introduce: _"Here's a visual breakdown of how your design scores across each category — red spots are your highest-priority fixes."_
...
- type: single_select
- options: "Show only 🔴 Critical" / "Show 🔴 + 🟡" / "Show everything" / "No filter"

Apply the filter and re-present only the relevant issue sections. Score and category breakdown always remain visible.

### Re-audit: Session Progress Tracker

Maintain a running audit history across the session. Every time an audit completes, record: overall score, accessibility score, issue counts, timestamp label.

**On re-audit**, open with a delta summary: _"Since the last audit: score improved from [X] → [Y]. 🔴 issues down from [N] to [N]. Here's what's new..."_

Show only **changed or new** issues — do not re-list resolved ones. Acknowledge wins: _"✅ Fixed since last audit: Color & Contrast (+3pts), States (+4pts)"_

**After the radar chart**, if 2+ audits exist, render a sparkline showing score history (e.g. 58 → 71 → 84).

---

## Step 4: Offer to Fix

After every report and radar chart, present a **"What next?" widget** using ask_user_input:
...
**If "Fix all Critical issues"** → loop through each 🔴 issue using the F5 fix workflow (Figma) or code diffs. For each issue, confirm before applying:

- ask_user_input: "Apply this fix? (Issue N of N)" — options: "Yes, apply it" / "Skip this one" / "Stop fixing"
- After all 🔴 issues resolved or skipped: "All critical fixes done. Want to continue with 🟡 warnings?"
- Never batch-apply all fixes without per-issue confirmation.

**If "Fix a specific issue"** → present a widget listing all 🔴 and 🟡 issues by name, let the user pick one, then show the before/after diff and apply.

**If "Show session progress"** → render the session sparkline widget. Only show this option if 2+ audits have been run.

**If "Developer handoff report"** → produce a clean markdown summary with: overall score, category table, critical issues with exact code fixes, and accessibility score. Copyable block for PR or Notion.

**If "Export report"** → create a downloadable `.md` file via file_create containing the full audit report.

Then respond based on selection. If dismissed: _"Want me to apply any of these fixes? I can edit the code directly, or if you're in Figma, I can make changes there too."_

**In Figma (🟢 High confidence, MCP active)**: Call `perform_editing_operations` → specific node IDs → verify with `get_screenshot` after each change. See F5 and `references/figma-mcp.md` for operation types and safety rules.
...
**In screenshots (🟡 Medium confidence)**: Never write code fixes for screenshot input — there is no source to edit. Give **design direction**: describe changes spatially, give target values as design specs, reference visual landmarks.
